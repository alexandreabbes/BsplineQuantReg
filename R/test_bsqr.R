...
#' @export
test_karlin_simple <- function(degree = 3,
                               seed = NULL,
                               verbose = FALSE)
{
  old <- par(mfrow = c(2, 2))
  ...
  if (verbose) {
    message("Simple Test. degree=:" , degree)
  }
  # Store par
  ...
  if (verbose) {
    message("\n===  PARTIAL INCREASING TEST ===\n
            Monotone constraints list =",
            monot)
  }

  res_croissant <- quantile_spline(
    xtab = xtab,
    ytab = ytab,
    knot = knot,
    tau = 0.5,
  ...
  res_decroissant <- quantile_spline(
    degree = degree,
    xtab = xtab,
    ytab = ytab,
    knot,
  ...
  der3cons[1:7] <- 0
  if (verbose) {
    message("3rd order constraints list:", der3cons)
  }
  res_3rd <- quantile_spline(
    xtab,
    ytab,
    knot,
    degree = degree,
